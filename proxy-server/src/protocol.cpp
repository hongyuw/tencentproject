...
int back_tcp_init(const char *ip_addr)
{
     ...
     * 设置接收缓冲区大小
     */
    rcv_size = SEND_BUFFER_SIZE; /* 接收缓冲区大小为4M */
    optlen = sizeof(rcv_size);
...
int tcp_receive(int client_fd, void *buf, int64_t n)
{
    ...
    if (ret < 0)
    {
        ...
        if (fdtab.count(client_fd)) //当前存在此fd键值对
        {
            ...
            if (fdtab.count(fdtab[client_fd]))
...
int tcp_send(int sock_fd, char *buffer, int64_t length)
{
    ...
    while (length > 0)
    {
        ...
        auto num = send(sock_fd, buffer, length, MSG_NOSIGNAL | MSG_DONTWAIT); //SIGPIPE and the output buffer is full return EAGAIN 先试一下加入DONTWAIT，或者加入判断EAGAIN，continue
        if (num < 0)
        {
