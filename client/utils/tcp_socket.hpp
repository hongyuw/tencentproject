...
public:
  ...
  bool Socket() {
    ...
    int re_t=setsockopt(_fd,SOL_SOCKET,SO_SNDTIMEO,(const char*)&timeout,sizeof(timeout));
    if (re_t <0 ) {
        printf("failed: time out of send\n");
    }
    re_t = 0;
    re_t=setsockopt(_fd,SOL_SOCKET,SO_RCVTIMEO,(const char*)&timeout,sizeof(timeout));
    if (re_t < 0) {
  ...
  bool Send(const void *buf, const ssize_t size) {
    ...
    while (bytes_need_to_send > 0) {
      ssize_t res = send(_fd, buf, bytes_need_to_send, MSG_NOSIGNAL);
